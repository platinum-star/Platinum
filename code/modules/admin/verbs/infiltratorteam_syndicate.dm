...
/client/proc/syndicate_infiltration_team()
	...
	var/input = null
	while(!input)
		input = sanitize_russian(copytext(input(src, "Please specify which mission the syndicate infiltration team shall undertake.", "Specify Mission", ""),1,MAX_MESSAGE_LEN))
		if(!input)
