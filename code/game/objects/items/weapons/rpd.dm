...
/obj/item/rpd/Topic(href, href_list, nowindow, state)
	...
	if(href_list["iconrotation"])
		iconrotation = text2num(sanitize_russian(href_list["iconrotation"]))
	else if(href_list["whatpipe"])
		whatpipe = text2num(sanitize_russian(href_list["whatpipe"]))
	else if(href_list["whatdpipe"])
		whatdpipe = text2num(sanitize_russian(href_list["whatdpipe"]))
	else if(href_list["pipe_category"])
		pipe_category = text2num(sanitize_russian(href_list["pipe_category"]))
	else if(href_list["mode"])
		mode = text2num(sanitize_russian(href_list["mode"]))
	else
		return
